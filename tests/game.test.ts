...
describe("게임 Unit test", () => {
  ...
  test("사용자가 입력한 숫자는 History에 저장되어야 한다.", () => {
    ...
    userInputs.forEach((userInput) => game.addToHistory(userInput));

    expect(game.history).toEqual(userInputs);
  });
  ...
  test(`사용자는 최대 ${MAX_RETRIES}번까지 정답을 맞출 수 있다.`, () => {
    ...
    expect(game.history.length).toBe(MAX_RETRIES);
  });
  ...
  test("사용자가 정한 재시도 횟수를 초과할 경우 에러가 발생한다.", () => {
    ...
    expect(() => {
      userInputs.forEach((userInput) => game.addToHistory(userInput));
    }).toThrow(ErrorMessage.game.EXCEED_MAX_RETRIES);
  });
});
